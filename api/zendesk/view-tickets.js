const { requireAuth } = require('../../lib/firebaseAuth');
const { zendeskFetch } = require('../../lib/zendeskClient');

module.exports = async function handler(req, res) {
  if (req.method !== 'GET') { res.status(405).json({ error: 'method not allowed' }); return; }

  try {
    await requireAuth(req);
  } catch (e) {
    res.status(401).json({ error: 'nao autenticado' });
    return;
  }

  const viewId = (req.query && req.query.view_id) || '';
  if (!viewId || !/^\d+$/.test(String(viewId))) {
    res.status(400).json({ error: 'view_id invalido' });
    return;
  }

  try {
    const zres = await zendeskFetch(`/api/v2/views/${viewId}/tickets.json`);
    if (!zres.ok) {
      const detail = await zres.text();
      res.status(zres.status).json({ error: 'erro na api do zendesk', detail });
      return;
    }
    const data = await zres.json();
    const tickets = (data.tickets || []).map(t => ({
      id: t.id,
      subject: t.subject,
      status: t.status,
      priority: t.priority,
      requester_id: t.requester_id,
      updated_at: t.updated_at,
      created_at: t.created_at,
      tags: t.tags,
    }));
    res.status(200).json({ tickets });
  } catch (e) {
    res.status(500).json({ error: 'erro interno', detail: e.message });
  }
};
