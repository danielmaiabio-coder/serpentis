...
module.exports = async function handler(req, res) {
  ...
  if (!q) { res.status(400).json({ error: 'informe um termo de busca' }); return; }

  try {
    const query = encodeURIComponent('type:ticket ' + q);
    const zres = await zendeskFetch(`/api/v2/search.json?query=${query}&sort_by=updated_at&sort_order=desc`);
    if (!zres.ok) {
